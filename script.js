    ...
    try {
        ...
        (data || []).forEach(profile => {
            ...
            if (email === currentUserEmail) {
                ...
                if (profile.email_visibility) localStorage.setItem(getEmailVisibilityKey(currentUserEmail), String(profile.email_visibility));
            }
        });
...
async function renderStoriesListForUser(targetEmail, listId) {
    ...
    if (!stories.length) {
        list.innerHTML = `<div class="stories-empty">${getCurrentLanguage() === 'en' ? 'No stories' : 'Нет историй'}</div>`;
        return;
    }
    ...
    list.innerHTML = stories.map((story) => {
        ...
        const timeText = createdAt.toLocaleString(getCurrentLanguage() === 'en' ? 'en-US' : 'ru-RU', {
            day: '2-digit',
            month: '2-digit',
            hour: '2-digit',
            minute: '2-digit'
        ...
        return `
            <button type="button" class="story-card" data-story-src="${safeSrc}" aria-label="Story">
                <img src="${safeSrc}" alt="story" class="story-card-image">
                <span class="story-card-time">${escapeHtml(timeText)}</span>
            </button>
        `;
    }).join('');
    ...
    list.querySelectorAll('.story-card').forEach((node) => {
        node.addEventListener('click', () => {
            const src = node.getAttribute('data-story-src') || '';
...
document.getElementById('login-form').addEventListener('submit', async function(e) {
    ...
    // Сначала пробуем локальный сценарий, затем fallback в Supabase Auth.
    ...
    if (isSupabaseEnabled()) {
        try {
            ...
            // Ниже покажем общее сообщение об ошибке входа.
        }
    }
