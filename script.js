...
async function loadSupabaseMessagesToLocalStore() {
    ...
    // Фильтруем только сообщения текущего пользователя
    data.filter(row => {
...
window.addEventListener('DOMContentLoaded', async () => {
    // Загружаем сохраненную тему (или белую по умолчанию)
    loadSavedTheme();
    
    // Инициализируем тему
    initThemeToggle();
    
    // Инициализируем модальное окно изображений
    initImageModal();
...
async function sendMessageToActiveConversation(text, fileData) {
    if (!currentUserEmail || !activeConversationId) return;
    ...
    // Если есть файл, то используем информацию о файле
    let messageBody = messageText;
    let attachedFile = null;
    
    if (fileData) {
        attachedFile = fileData;
        messageBody = messageText || `📎 ${fileData.name}`;
    }
    ...
    if (client && supabaseMessagesTableAvailable) {
        ...
        if (error) {
            if (isSupabaseMessagesTableMissing(error)) {
                ...
                appendLocalMessageToActiveConversation(text, fileData);
                return;
            }
