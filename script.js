...
function initChatSubmenu() {
    const inner = document.querySelector('.chat-submenu-inner');
    if (!inner) return;

    // create indicator element if not present
    ...
    const buttons = Array.from(inner.querySelectorAll('.submenu-btn'));
    ...
    function moveTo(btn) {
        ...
        // Make the indicator clearly smaller so it never touches the container edges.
        // Increase reduction so the pill fits comfortably under the button.
        const reduce = Math.min(40, Math.round(btnWidth * 0.18));
        let width = Math.max(24, btnWidth - reduce);
        let left = btnLeft + Math.round((btnWidth - width) / 2);

        // Clamp inside container to prevent overflow at edges
        const maxLeft = Math.max(0, inner.clientWidth - width - 2);
        if (left < 2) left = 2;
    ...
    // Initial active: `Чаты`
    const defaultBtn = inner.querySelector('.submenu-btn[data-tab="chats"]') || buttons[0];
    buttons.forEach(b => b.classList.remove('active'));
    defaultBtn.classList.add('active');
    // position indicator after layout
    requestAnimationFrame(() => moveTo(defaultBtn));

    buttons.forEach(btn => {
        btn.addEventListener('click', () => {
            moveTo(btn);
            // emit custom event so other code can react if needed
            const ev = new CustomEvent('chatSubmenuChange', { detail: { tab: btn.getAttribute('data-tab') } });
...
function createMessageBubbleMarkup(message, peerEmail) {
    ...
    return `
        <div class="message ${isOutgoing ? 'outgoing' : 'incoming'}" data-message-id="${escapeHtml(message.id || '')}" data-sender="${escapeHtml(message.sender || '')}">
...
function renderConversationThread(chatArea, conversation) {
    ...
    if (!visibleMessages.length) {
        ...
        messagesContainer.innerHTML = visibleMessages.map(message => createMessageBubbleMarkup(message, peerEmail)).join('');
    }
