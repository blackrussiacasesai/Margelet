...
function renderConversationThread(chatArea, conversation) {
    ...
    if (chatArea.dataset.threadConversationId === conversation.id && activeThreadDom && lastRenderedConversationSignature === nextSignature) {
    ...
    if (!activeThreadDom || chatArea.dataset.threadMounted !== conversation.id) {
        chatArea.innerHTML = `
