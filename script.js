...
const MARGELET_NOTIFICATIONS_EMAIL = 'margelet.notifications@system.local';
const MARGELET_NOTIFICATIONS_TITLE = 'Margelet notifications';
const MARGELET_NOTIFICATIONS_AVATAR = 'https://i.imgur.com/Sx7pmLM.png';
...
async function syncSupabaseProfilesToLocalDirectory(force = false) {
    ...
    try {
        ...
        (data || []).forEach(profile => {
            ...
            const hasBoostFromProfile = Boolean(profile.premium_gold ?? existing.hasGold);
            usersByEmail.set(email, {
                ...
                nicknameGradientEnabled: hasBoostFromProfile && (profile.nickname_gradient_enabled === true || Boolean(profile.nickname_gradient_enabled ?? existing.nicknameGradientEnabled ?? false)),
                ...
                avatarAnimationEnabled: hasBoostFromProfile && !(profile.avatar_animation_enabled === false || profile.avatar_animation_enabled === '0'),
                avatarGlowEnabled: hasBoostFromProfile && !(profile.avatar_glow_enabled === false || profile.avatar_glow_enabled === '0'),
            ...
            if (email === currentUserEmail) {
                ...
                const currentHasBoost = Boolean(profile.premium_gold ?? getMembershipState().gold);
                if (!currentHasBoost) {
                    localStorage.setItem('avatarAnimationEnabled_' + email, '0');
                    localStorage.setItem('avatarGlowEnabled_' + email, '0');
                    localStorage.setItem(getNicknameGradientKey(email), '0');
                }
...
function applyGradientToNameElement(element, email) {
    ...
    const canShowGradient = Boolean(profileData.nicknameGradientEnabled);
...
function loadEditData() {
    ...
    const savedGlowAnimation = normalizeGlowAnimation(localStorage.getItem('glowAnimation_' + currentUserEmail));
    ...
    if (editAvatarAnimationEnabledInput) {
        ...
        const row = editAvatarAnimationEnabledInput.closest('.editor-v4-row');
        if (row) row.classList.toggle('boost-feature-locked', !hasBoost);
    }
    ...
    // Цвет и анимация glow настраиваются только с Boost.
    setGlowColor(hasBoost ? savedGlowColor : 'gradient');
    setGlowAnimation(hasBoost ? savedGlowAnimation : 'pulse');
    if (glowExpandBtn) {
        glowExpandBtn.disabled = !hasBoost;
        glowExpandBtn.setAttribute('aria-disabled', !hasBoost ? 'true' : 'false');
        glowExpandBtn.classList.toggle('disabled', !hasBoost);
    }
    if (glowAnimExpandBtn) {
        glowAnimExpandBtn.disabled = !hasBoost;
        glowAnimExpandBtn.setAttribute('aria-disabled', !hasBoost ? 'true' : 'false');
        glowAnimExpandBtn.classList.toggle('disabled', !hasBoost);
    }
...
function saveEditData() {
    ...
    // Сохраняем цвет/анимацию glow только для Boost.
    const selectedColor = document.querySelector('.glow-color-option.selected');
    if (selectedColor) {
        const color = hasBoost ? selectedColor.getAttribute('data-color') : 'gradient';
        localStorage.setItem('glowColor_' + currentUserEmail, color);
    }
    if (!hasBoost) {
        localStorage.setItem('glowAnimation_' + currentUserEmail, 'pulse');
    }
...
function completeSuccessfulLogin(email, displayName = '') {
    ...
    ensureMargeletNotificationsConversation();
    document.documentElement.classList.add('logged-in');
    addAccountToLoggedList(currentUserEmail);
    goToMainScreen();
}
...
function updateChatArea(options = {}) {
    ...
    if (!currentUserEmail) {
        ...
        if (fileDraftNode) fileDraftNode.classList.add('hidden');
    ...
    if (activeConversationId) {
        ...
        if (canShowConversation) {
            ...
            const isReadonlySystemChat = isMargeletNotificationsConversation(conversation) || conversation.readOnly === true;
            chatForm.classList.toggle('hidden', isReadonlySystemChat);
            if (fileDraftNode) {
                fileDraftNode.classList.toggle('hidden', isReadonlySystemChat);
                if (isReadonlySystemChat) fileDraftNode.innerHTML = '';
            }
            updateBottomNavigationVisibility();
            return;
        }
...
function renderCommunitiesList(chatArea) {
    ...
    async function reloadCommunitiesOnline() {
        ...
        allCommunities = allCommunities.filter((community) => !String(community?.id || '').startsWith('demo-'));
...
function getMargeletNotificationsConversationId(email = currentUserEmail) {
    const normalized = normalizeEmail(email || '');
    if (!normalized) return '';
    return 'margelet_notifications__' + normalized;
}

function isMargeletNotificationsConversation(conversation) {
    if (!conversation) return false;
    if (conversation.systemKey === 'margelet-notifications') return true;
    return String(conversation.id || '').startsWith('margelet_notifications__');
}

function ensureMargeletNotificationsConversation() {
    if (!currentUserEmail) return null;

    upsertUserInLocalDirectory(MARGELET_NOTIFICATIONS_EMAIL, MARGELET_NOTIFICATIONS_TITLE);
    localStorage.setItem('userAvatar_' + MARGELET_NOTIFICATIONS_EMAIL, MARGELET_NOTIFICATIONS_AVATAR);

    const conversationId = getMargeletNotificationsConversationId(currentUserEmail);
    ...
    if (!store[conversationId]) {
        ...
        store[conversationId] = {
            id: conversationId,
            type: 'system',
            title: MARGELET_NOTIFICATIONS_TITLE,
            avatar: MARGELET_NOTIFICATIONS_AVATAR,
            participants: [normalizeEmail(currentUserEmail), MARGELET_NOTIFICATIONS_EMAIL],
            ownerEmail: MARGELET_NOTIFICATIONS_EMAIL,
            admins: [MARGELET_NOTIFICATIONS_EMAIL],
            readOnly: true,
            systemKey: 'margelet-notifications',
            messages: [
                {
                    id: 'system-welcome-' + now,
                    sender: MARGELET_NOTIFICATIONS_EMAIL,
                    text: 'Добро пожаловать! Здесь будут новости Margelet и важные уведомления. Ответы в этом чате отключены.',
                    timestamp: now,
                    deleted_for: [],
                    read_by: [normalizeEmail(currentUserEmail)]
                }
            ],
            updatedAt: now
        };
...
function getConversationDisplayMeta(conversation) {
    if (isMargeletNotificationsConversation(conversation)) {
        return {
            isGroup: false,
            peerEmail: MARGELET_NOTIFICATIONS_EMAIL,
            profileOpenAllowed: false,
            title: MARGELET_NOTIFICATIONS_TITLE,
            subtitle: 'Новости и системные уведомления',
            avatar: conversation?.avatar || getAvatarByEmail(MARGELET_NOTIFICATIONS_EMAIL) || MARGELET_NOTIFICATIONS_AVATAR,
...
function createAvatarMarkup(user, options = {}) {
    ...
    const triggerClass = clickable ? ' user-profile-trigger' : '';
    const fallbackClass = user.avatar ? `chat-avatar${triggerClass}` : `chat-avatar fallback${triggerClass}`;
    ...
    conversations.forEach(conversation => {
        ...
        const avatarHtml = meta.isGroup
            ...
            : createAvatarMarkup({
                ...
                clickable: meta.profileOpenAllowed !== false
            });
        ...
        const avatarTrigger = row.querySelector('.user-profile-trigger');
        if (avatarTrigger && !meta.isGroup && meta.peerEmail && meta.profileOpenAllowed !== false) {
            avatarTrigger.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
...
function renderConversationThread(chatArea, conversation) {
    ...
    if (!isActiveThreadDomValid || chatArea.dataset.threadMounted !== conversation.id) {
        chatArea.innerHTML = `
            <div class="thread-view">
                <div class="thread-header">
                    ...
                    <div class="thread-peer-avatar ${meta.isGroup || meta.profileOpenAllowed === false ? '' : 'user-profile-trigger'} ${peerAvatar ? '' : 'fallback'}" data-email="${escapeHtml(peerEmail || '')}" ${avatarStyle}>${peerAvatar ? '' : escapeHtml(peerFirstLetter)}</div>
        ...
        if (avatarNode) {
            avatarNode.className = `thread-peer-avatar ${meta.isGroup || meta.profileOpenAllowed === false ? '' : 'user-profile-trigger'} ${peerAvatar ? '' : 'fallback'}`.trim();
...
async function sendMessageToActiveConversation(text, fileData, forcedConversationId = null) {
    ...
    if (isMargeletNotificationsConversation(conversation) || conversation.readOnly === true) {
        alert('В чат Margelet notifications нельзя писать. Здесь публикуются только новости и системные уведомления.');
        return false;
    }
...
function setGlowColor(color) {
    ...
    if (currentUserEmail && !hasMargeletBoost(currentUserEmail)) {
        color = 'gradient';
    }
...
document.querySelectorAll('.glow-anim-option').forEach(option => {
    option.addEventListener('click', function() {
        if (!hasMargeletBoost(currentUserEmail)) {
            alert('Анимация свечения доступна только с подпиской Margelet Boost.');
            return;
        }
...
if (glowColorOptions.length > 0) {
    glowColorOptions.forEach(option => {
        option.addEventListener('click', function() {
            if (!hasMargeletBoost(currentUserEmail)) {
                alert('Цвет свечения доступен только с подпиской Margelet Boost.');
                return;
            }
