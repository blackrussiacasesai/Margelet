...
function getConversationDisplayMeta(conversation) {
    ...
    const avatar = firstPeer ? getAvatarByEmail(firstPeer) : null;
    return {
        isGroup: true,
        peerEmail: null,
        title,
...
function createGroupConversation(participantEmails, title = '') {
...
function createGroupAvatarMarkup(title = '') {
...
function renderConversationsList(chatArea) {
    ...
    conversations.forEach(conversation => {
        ...
        const avatarHtml = meta.isGroup
            ? createGroupAvatarMarkup(meta.title)
            : createAvatarMarkup({
...
function renderConversationThread(chatArea, conversation) {
    ...
    if (!isActiveThreadDomValid || chatArea.dataset.threadMounted !== conversation.id) {
        ...
        bindThreadInteractions(chatArea, meta.isGroup ? '' : peerEmail);
    } else {
...
function initMessaging() {
    ...
    if (groupCreateSubmitBtn && groupCreateSubmitBtn.dataset.bound !== '1') {
        groupCreateSubmitBtn.addEventListener('click', () => {
            ...
            const groupConversation = createGroupConversation(participants, title);
