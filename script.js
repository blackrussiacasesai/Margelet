...
function updateUserProfileContactAction() {
    ...
    if (label) {
        label.textContent = added
            ? (lang === 'en' ? 'In contacts' : 'В контактах')
...
function createAvatarMarkup(user) {
    ...
    const glowRaw = user.glowColor || user.glow_color || getUserProfileDataByEmail(user.email || '').glowColor;
    const glowColor = getGlowColorValue(glowRaw || 'gradient');
    const avatarStyle = user.avatar
        ? `style="background-image:url('${escapeHtml(user.avatar)}'); box-shadow: 0 0 0 1px rgba(255,255,255,0.08), 0 0 18px ${escapeHtml(glowColor)}55;"`
        : `style="box-shadow: 0 0 0 1px rgba(255,255,255,0.08), 0 0 18px ${escapeHtml(glowColor)}55;"`;
...
function initNavigation() {
    ...
    window.updateActiveNavItem = function(screenId) {
        const settingsScreens = new Set(['screen-settings', 'screen-notifications', 'screen-devices', 'screen-languages', 'screen-wallet', 'screen-services']);
        const effectiveScreenId = screenId === 'screen-user-profile'
            ? 'screen-main'
...
function setGlowColor(color) {
    ...
    if (profileGlow) {
        ...
        profileGlow.style.setProperty('--fog-c1', fogPalette[0]);
        profileGlow.style.setProperty('--fog-c2', fogPalette[1]);
        profileGlow.style.setProperty('--fog-c3', fogPalette[2]);
    }
}
...
function setGlowAnimation(animType) {
    ...
    const profileGlow = document.getElementById('user-profile-avatar-glow');
    if (profileGlow) {
        profileGlow.style.animation = animValue;
        profileGlow.setAttribute('data-anim', animType);
    }
}
...
function applyLanguage(lang) {
    ...
    setText('#screen-wallet .wallet-actions .wallet-action-btn:nth-child(1)', tr.walletTopup);
    setText('#screen-wallet .wallet-actions .wallet-action-btn:nth-child(2)', tr.walletTransfer);
    setText('#screen-wallet .wallet-actions .wallet-action-btn:nth-child(3)', tr.walletHistory);
    ...
    setText('#screen-wallet .wallet-history-row:nth-child(1) .wallet-history-title', tr.walletOpTopup);
    setText('#screen-wallet .wallet-history-row:nth-child(2) .wallet-history-title', tr.walletOpSubscription);
    setText('#screen-wallet .wallet-history-row:nth-child(3) .wallet-history-title', tr.walletOpTransferFromContact);
    setText('#screen-wallet .wallet-history-row:nth-child(1) .wallet-history-meta', tr.walletToday);
    setText('#screen-wallet .wallet-history-row:nth-child(2) .wallet-history-meta', tr.walletYesterdayOne);
    setText('#screen-wallet .wallet-history-row:nth-child(3) .wallet-history-meta', tr.walletYesterdayTwo);
...
document.querySelectorAll('#premium-card, #business-card').forEach(btn => {
    if (btn) {
        btn.addEventListener('click', () => {
