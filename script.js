...
function scheduleChatAreaUpdate(options = {}) {
    pendingChatAreaUpdateOptions = options;
    ...
    chatAreaUpdateRafId = requestAnimationFrame(() => {
        ...
        const nextOptions = pendingChatAreaUpdateOptions || {};
        pendingChatAreaUpdateOptions = null;
        updateChatArea(nextOptions);
    });
}
...
async function loadSupabaseMessagesToLocalStore() {
    ...
    try {
        const result = await client
            .from('messages_app')
            .select('id, conversation_id, participants, sender_email, body, sent_at, deleted_for, file')
    ...
    supabaseMessagesChannel = client
        ...
        .on(
            ...
            (payload) => {
                ...
                const store = getConversationsStore();
                for (const convId in store) {
                    const conversation = store[convId];
        ...
        .on(
            ...
            (payload) => {
                ...
                // Удалить сообщение из локального хранилища
                const store = getConversationsStore();
                for (const convId in store) {
                    const conversation = store[convId];
...
async function registerCurrentPendingUser() {
    ...
    try {
        ...
        // Сохраняем в профиль
        currentUserEmail = normalizeEmail(currentRegistrationEmail);
        localStorage.setItem('currentUserEmail', currentUserEmail);
...
function updateChatArea(options = {}) {
    const { skipRemoteRefresh = false } = options;
...
async function refreshUserPresenceMap() {
    if (!isSupabaseEnabled() || !currentUserEmail) return;
    ...
    try {
        ...
        await Promise.all(
            emailsToCheck.map(async (email) => {
                try {
                    const isOnline = await window.checkUserOnlineStatus(email);
                    presence[email] = Boolean(isOnline);
                } catch (_error) {
                    presence[email] = false;
                }
            })
        );
        ...
        const screenMain = document.getElementById('screen-main');
        if (screenMain && screenMain.classList.contains('active') && activeConversationId) {
            scheduleChatAreaUpdate({ skipRemoteRefresh: true });
        }
    } catch (_error) {
        // noop
...
function startUserPresencePolling() {
    ...
    setInterval(() => {
        if (document.hidden) return;
        if (!currentUserEmail || !isSupabaseEnabled()) return;
        refreshUserPresenceMap();
    }, 15000);
}
...
function renderConversationThread(chatArea, conversation) {
    ...
    if (!conversation.messages.length) {
        ...
        conversation.messages.forEach(message => {
            // Пропустить если сообщение удалено для текущего пользователя
            if (message.deleted_for && message.deleted_for.includes(currentUserEmail)) {
                return;
            }
            ...
            const dateString = timestamp.toLocaleDateString('ru-RU', {
                day: 'numeric',
                month: 'short'
            });

            // Показать дату если это не сегодня
            const today = new Date();
            const isToday = timestamp.toDateString() === today.toDateString();
            const fullTimeString = isToday ? timeString : dateString + ' ' + timeString;

            const bubble = document.createElement('div');
            bubble.className = `message ${isOutgoing ? 'outgoing' : 'incoming'}`;
            bubble.setAttribute('data-message-id', message.id || '');
            bubble.setAttribute('data-sender', message.sender || '');
            
            // Содержимое сообщения с меню удаления
            ...
            if (message.file) {
                ...
                // Отобразить превью для изображений, видео и т.д.
                if (fileData.type.startsWith('image/')) {
                    ...
                    img.addEventListener('click', () => {
                        ...
                        if (modal && modalImg) {
                            modalImg.src = fileData.data;
                            modalImg.alt = fileData.name;
                            modal.classList.add('active');
                            
                            // Обновить кнопку скачивания
                            if (modalDownloadBtn) {
                                modalDownloadBtn.href = fileData.data;
                                modalDownloadBtn.download = fileData.name;
                            }
                        }
                    });
                    ...
                    video.className = 'message-file-preview';
                    video.controls = true;
                    video.src = fileData.data;
                    fileContainer.appendChild(video);
                } else if (fileData.type.startsWith('audio/')) {
                    const audio = document.createElement('audio');
                    audio.className = 'message-file-preview';
                    audio.controls = true;
                ...
                // Добавить кнопку скачивания (видна при наведении)
                const downloadBtn = document.createElement('a');
                downloadBtn.className = 'message-file-download';
                downloadBtn.href = fileData.data;
                downloadBtn.download = fileData.name;
                downloadBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Скачать';
            ...
            timeSpan.textContent = fullTimeString;
            ...
            // Add long-press functionality for message deletion
            if (message.id) {
                let pressTimer;
                let isPressValid = true;
                ...
                const startPress = () => {
                    ...
                    pressTimer = setTimeout(() => {
                        if (isPressValid) {
                            ...
                            showMessageDeleteMenu(message.id, message.sender, peerEmail);
                        }
                ...
                bubble.addEventListener('touchmove', movePress);
            }
        });
    }
...
function initThemeToggle() {
    ...
    const themeBtn = document.getElementById('action-theme');
