...
async function sendWebPushForMessage(conversation, messageRow) {
    ...
    try {
        await client.functions.invoke('send-web-push', {
            body: {
...
async function syncCurrentUserProfileToSupabase() {
    ...
    let payload = {
        ...basePayload,
        premium_gold: Boolean(membership.gold),
    ...
    if (error) {
        const details = String(error?.details || '').toLowerCase();
        const message = String(error?.message || '').toLowerCase();
        const missingExtraColumns = details.includes('premium_gold') || details.includes('gold_badge') || details.includes('visibility') || details.includes('login_pin') || message.includes('premium_gold') || message.includes('gold_badge') || message.includes('visibility') || message.includes('login_pin');
...
async function syncSupabaseProfilesToLocalDirectory(force = false) {
    ...
    try {
        let { data, error } = await client
            .from('profiles')
            .select('email, display_name, username, phone, avatar_url, about, birthday, glow_color, email_visibility, phone_visibility, avatar_visibility, status_visibility, birthday_visibility, login_pin, last_seen_at, premium_gold, gold_badge_visible, gold_badge_style')
...
function getCollectionTypeConfig() {
    ...
    return {
        ...
        'crystal': {
            label: getCurrentLanguage() === 'en' ? 'Crystal' : 'Кристалл',
            value: 5299,
            names: getCurrentLanguage() === 'en' ? ['Crystal'] : ['Кристалл']
        }
    };
}
...
function buildCollectionAppleMarkup() {
    return `
        <div class="collection-item-apple-shell" aria-hidden="true">
            <svg viewBox="0 0 100 100" class="collection-item-apple-svg" preserveAspectRatio="xMidYMid meet">
                <defs>
                    <linearGradient id="appleLiquidA" x1="0" y1="0" x2="1" y2="1">
                        <stop offset="0%" stop-color="#f4f7ff">
                            <animate attributeName="stop-color" values="#f4f7ff;#ffe2f3;#dff1ff;#f4f7ff" dur="4.6s" repeatCount="indefinite" />
                        </stop>
                        <stop offset="100%" stop-color="#c8dcff">
                            <animate attributeName="stop-color" values="#c8dcff;#a8c9ff;#d7b7ff;#c8dcff" dur="4.2s" repeatCount="indefinite" />
                        </stop>
                    </linearGradient>
                    <radialGradient id="appleLiquidB" cx="50%" cy="45%" r="52%">
                        <stop offset="0%" stop-color="rgba(255,255,255,0.95)"/>
                        <stop offset="55%" stop-color="rgba(197,226,255,0.78)"/>
                        <stop offset="100%" stop-color="rgba(173,194,255,0.28)"/>
                    </radialGradient>
                    <filter id="appleGlow" x="-40%" y="-40%" width="180%" height="180%">
                        <feGaussianBlur stdDeviation="2.8" result="blur"/>
                        <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
                    </filter>
                    <mask id="appleMask">
                        <rect x="0" y="0" width="100" height="100" fill="black"/>
                        <path fill="white" d="M51.1,38.2 C51.1,28.7 58.8,24 59.2,23.7 C54.8,17.3 47.9,16.3 45.5,16.1 C39.6,15.5 33.9,19.6 30.9,19.6 C27.9,19.6 23.2,16.1 18.2,16.2 C11.6,16.3 5.6,20.1 2.2,26 C-4.8,38 0.4,55.8 7.2,65.6 C10.5,70.4 14.4,75.7 19.5,75.5 C24.4,75.3 26.2,72.3 32,72.3 C37.8,72.3 39.5,75.5 44.7,75.4 C50.1,75.3 53.5,70.6 56.8,65.7 C60.6,59.8 62.1,54.1 62.2,53.8 C62.1,53.7 51.2,49.5 51.1,38.2 Z M39.6,4.5 C41.5,2.2 42.8,-1.0 42.4,-4.2 C39.4,-4.1 36,-2.2 34.1,0.1 C32.3,2.3 30.8,5.6 31.3,8.8 C34.6,9.1 37.7,6.9 39.6,4.5 Z"/>
                    </mask>
                </defs>
                <g transform="translate(20 12) scale(0.9)">
                    <g mask="url(#appleMask)">
                        <rect x="-20" y="-20" width="160" height="160" fill="url(#appleLiquidA)">
                            <animateTransform attributeName="transform" type="translate" values="-10 -6; 8 4; -10 -6" dur="6.2s" repeatCount="indefinite"/>
                        </rect>
                        <ellipse cx="52" cy="42" rx="28" ry="21" fill="url(#appleLiquidB)" opacity="0.78" filter="url(#appleGlow)">
                            <animate attributeName="rx" values="26;30;26" dur="3.1s" repeatCount="indefinite"/>
                            <animate attributeName="ry" values="20;24;20" dur="3.4s" repeatCount="indefinite"/>
                            <animate attributeName="cy" values="42;46;42" dur="3.7s" repeatCount="indefinite"/>
                        </ellipse>
                    </g>
                    <path class="apple-outline" d="M51.1,38.2 C51.1,28.7 58.8,24 59.2,23.7 C54.8,17.3 47.9,16.3 45.5,16.1 C39.6,15.5 33.9,19.6 30.9,19.6 C27.9,19.6 23.2,16.1 18.2,16.2 C11.6,16.3 5.6,20.1 2.2,26 C-4.8,38 0.4,55.8 7.2,65.6 C10.5,70.4 14.4,75.7 19.5,75.5 C24.4,75.3 26.2,72.3 32,72.3 C37.8,72.3 39.5,75.5 44.7,75.4 C50.1,75.3 53.5,70.6 56.8,65.7 C60.6,59.8 62.1,54.1 62.2,53.8 C62.1,53.7 51.2,49.5 51.1,38.2 Z M39.6,4.5 C41.5,2.2 42.8,-1.0 42.4,-4.2 C39.4,-4.1 36,-2.2 34.1,0.1 C32.3,2.3 30.8,5.6 31.3,8.8 C34.6,9.1 37.7,6.9 39.6,4.5 Z" />
                </g>
            </svg>
        </div>
    `;
...
function buildCollectionCrystalMarkup() {
    const doc = `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><script type="importmap">{"imports":{"three":"https://unpkg.com/three@0.183.0/build/three.webgpu.js","three/webgpu":"https://unpkg.com/three@0.183.0/build/three.webgpu.js","three/tsl":"https://unpkg.com/three@0.183.0/build/three.tsl.js","three/addons/":"https://unpkg.com/three@0.183.0/examples/jsm/"}}</script><style>body{background:transparent;overflow:hidden;margin:0}#webgpu-canvas{width:100vw;height:100vh;display:block}</style></head><body><canvas id="webgpu-canvas"></canvas><script type="module">import * as THREE from 'three/webgpu';import { OrbitControls } from 'three/addons/controls/OrbitControls.js';import { ConvexGeometry } from 'three/addons/geometries/ConvexGeometry.js';const scene=new THREE.Scene();const camera=new THREE.PerspectiveCamera(75,innerWidth/innerHeight,0.1,10);camera.position.z=2;const renderer=new THREE.WebGPURenderer({canvas:document.querySelector('#webgpu-canvas'),antialias:true,alpha:true});renderer.setClearColor(0x000000,0);renderer.setSize(innerWidth,innerHeight);const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;function mulberry32(seed){return function(){seed|=0;seed=(seed+0x6d2b79f5)|0;let t=Math.imul(seed^(seed>>>15),1|seed);t=(t+Math.imul(t^(t>>>7),61|t))^t;return ((t^(t>>>14))>>>0)/4294967296;};}function generateGemPoints(count,seed){const rng=mulberry32(seed);const points=[];points.push(new THREE.Vector3(0,0.83,0));points.push(new THREE.Vector3(0,-1.3,0));for(let i=0;i<count-2;i++){const theta=rng()*Math.PI*2;const raw=rng()*2-1;const y=Math.sign(raw)*Math.pow(Math.abs(raw),1/1.15)*0.85+(rng()-0.5)*1;const r=0.7*(1-Math.abs(y));points.push(new THREE.Vector3(Math.cos(theta)*r,y,Math.sin(theta)*r));}return points;}const geometry=new ConvexGeometry(generateGemPoints(128,3695));const exteriorMat=new THREE.MeshPhysicalMaterial({color:0x8bc7ff,transmission:0.92,thickness:1.1,roughness:0.02,metalness:0.03,iridescence:1,iridescenceIOR:1.5,clearcoat:1,clearcoatRoughness:0.05});const interiorMat=new THREE.MeshBasicMaterial({color:0xff58d4,transparent:true,opacity:0.30,side:THREE.BackSide,blending:THREE.AdditiveBlending,depthWrite:false,depthTest:false});const exteriorMesh=new THREE.Mesh(geometry,exteriorMat);const interiorMesh=new THREE.Mesh(geometry,interiorMat);const edgeMesh=new THREE.LineSegments(new THREE.EdgesGeometry(geometry,14),new THREE.LineBasicMaterial({color:0x8cc7ff,transparent:true,opacity:0.34}));interiorMesh.renderOrder=1;edgeMesh.renderOrder=2;scene.add(interiorMesh,exteriorMesh,edgeMesh);const lightA=new THREE.DirectionalLight(0x58b3ff,2.0);lightA.position.set(2,2,2);scene.add(lightA);const lightB=new THREE.DirectionalLight(0xff63d6,1.6);lightB.position.set(-2,-1,1);scene.add(lightB);const lightC=new THREE.DirectionalLight(0x9e6bff,1.4);lightC.position.set(0,-2,2);scene.add(lightC);addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);});renderer.setAnimationLoop(()=>{controls.update();const t=performance.now()*0.00035;exteriorMat.color.setHSL(0.55+0.25*Math.sin(t*1.3),0.9,0.7);interiorMat.color.setHSL(0.78+0.18*Math.cos(t*1.7),0.95,0.6);edgeMesh.material.color.setHSL(0.6+0.06*Math.sin(t*1.1),0.85,0.62);exteriorMesh.rotation.y+=0.0022;exteriorMesh.rotation.x=0.18*Math.sin(t*2.0);interiorMesh.rotation.y=exteriorMesh.rotation.y;interiorMesh.rotation.x=exteriorMesh.rotation.x;edgeMesh.rotation.y=exteriorMesh.rotation.y;edgeMesh.rotation.x=exteriorMesh.rotation.x;renderer.render(scene,camera);});</script></body></html>`;
    return `
        <div class="collection-item-iframe-wrap" aria-hidden="true">
            <iframe class="collection-item-iframe" loading="lazy" title="Crystal item" tabindex="-1" srcdoc="${escapeHtml(doc)}"></iframe>
        </div>
    `;
}
...
function getCollectionObjectMarkup(item, isDetailed = false) {
    ...
    if (itemType === 'apple') return buildCollectionAppleMarkup();
    if (itemType === 'crystal') return buildCollectionCrystalMarkup();
...
function resizeAvatarParticleState(state) {
    ...
    const base = Math.max(wrapper.clientWidth || 0, wrapper.clientHeight || 0, 112);
    const drawSize = Math.round(base * 1.95);
...
function initAvatarParticleAnimation() {
    ...
    const animate = (now) => {
        ...
        if (document.documentElement.classList.contains('avatar-effects-off')) {
...
window.addEventListener('DOMContentLoaded', () => {
    bootstrapApp();
    const currentEffectsEnabled = currentUserEmail
        ? localStorage.getItem('avatarEffectsEnabled_' + currentUserEmail) !== '0'
        : true;
    document.documentElement.classList.toggle('avatar-effects-off', !currentEffectsEnabled);
});
...
function updateMembershipUI() {
    ...
    if (premiumCard) {
        ...
        if (title) {
            title.textContent = 'Margelet Gold';
        }
    }
...
async function changeEmailInPrivacy(newEmailRaw) {
    ...
    ['userDisplayName_', 'userName_', 'userPhone_', 'userBirthday_', 'userStatus_', 'userAvatar_', 'glowColor_', 'margeletMembership_', 'margeletGoldBadgeVisible_', 'margeletGoldBadgeStyle_', 'margeletContacts_', 'margeletBlockedUsers_', 'deviceSessions_'].forEach(copyKey);
...
const editAvatarEffectsEnabledInput = document.getElementById('edit-avatar-effects-enabled');

let editSnapshot = null;

function getAvatarEffectsEnabled() {
    if (!currentUserEmail) return true;
    return localStorage.getItem('avatarEffectsEnabled_' + currentUserEmail) !== '0';
}

function applyAvatarEffectsEnabled(enabled) {
    const isEnabled = Boolean(enabled);
    document.documentElement.classList.toggle('avatar-effects-off', !isEnabled);
    if (isEnabled) return;
    avatarParticleStates.forEach((state) => {
...
function loadEditData() {
    ...
    const savedAvatarEffectsEnabled = getAvatarEffectsEnabled();
    ...
    if (editAvatarEffectsEnabledInput) {
        editAvatarEffectsEnabledInput.checked = savedAvatarEffectsEnabled;
    }
    ...
    applyAvatarEffectsEnabled(savedAvatarEffectsEnabled);
...
function saveEditData() {
    ...
    if (editAvatarEffectsEnabledInput) {
        const effectsEnabled = Boolean(editAvatarEffectsEnabledInput.checked);
        localStorage.setItem('avatarEffectsEnabled_' + currentUserEmail, effectsEnabled ? '1' : '0');
        applyAvatarEffectsEnabled(effectsEnabled);
    }
...
function openUserProfileScreenByEmail(email, sourceScreenId = 'screen-main') {
    ...
    if (profileGlow) {
        ...
        profileGlow.style.display = canSeeAvatar ? 'block' : 'none';
    }
...
function renderConversationsList(chatArea, options = {}) {
    ...
    conversations.forEach(conversation => {
        ...
        row.innerHTML = `
            ...
            <div class="chat-list-content">
                <div class="chat-list-top">
                    <span class="chat-list-name">${escapeHtml(meta.title)}</span>
    ...
    members.forEach((member) => {
        ...
        row.innerHTML = `
            ...
            <div class="device-item-main group-candidate-main">
                <div class="device-item-title">${escapeHtml(member.displayName)}</div>
...
function renderConversationAdminsScreen() {
    ...
    members.forEach((email) => {
        ...
        row.innerHTML = `
            ...
            <div class="device-item-main group-candidate-main">
                <div class="device-item-title">${escapeHtml(getDisplayNameByEmail(email))}</div>
...
function createUsernameDisplay() {
    ...
    if (isGoldBadgeVisible()) {
        ...
        badge.title = 'Margelet Gold';
        badge.setAttribute('aria-label', 'Margelet Gold');
        badge.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 10h12"></path><path d="m8 10 4-5 4 5"></path><path d="m9 10 3 9 3-9"></path></svg>';
...
if (goldBuyBtn) {
    goldBuyBtn.addEventListener('click', () => {
        ...
        updateMembershipUI();
        alert('Margelet Gold подключен. Списано 199 руб.');
    });
}
