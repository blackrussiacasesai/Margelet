...
// Старая версия - оставляем только для обратной совместимости
if (false) {
    const client = getSupabaseClient();
    if (client) {
    ...
    if (false && !tryLegacyLocalLogin(email, password)) {
...
async function registerCurrentPendingUser() {
    ...
    try {
        ...
        // Добавляем в базу profiles для поиска других пользователей
        const client = getSupabaseClient();
        if (client) {
            try {
                ...
                }]).on('*', m => {});  // Игнорируем ошибки если уже есть
            } catch (err) {
                console.log('Профиль уже в базе или ошибка добавления');
            }
        }
