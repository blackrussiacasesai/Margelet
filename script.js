...
function playSendMessageSound() {
    try {
        ...
        if (Notification.permission === 'denied') {
            return; // Не воспроизводим звук если уведомления отключены
        }
        
        // Если разрешение не установлено, запрашиваем его ТОЛЬКО один раз
        if (Notification.permission === 'default' && !notificationPermissionRequested) {
            notificationPermissionRequested = true;
            localStorage.setItem('notificationPermissionRequested', 'true');
            Notification.requestPermission().catch(() => {
                // Игнорируем ошибки при запросе разрешения
            });
            return; // Не воспроизводим звук при первом запросе
        }
        ...
        // Первый клик - низкий приглушенный, потише
        ...
        // Второй клик - чуть выше, тоже приглушен и потише
        ...
        // Браузер не поддерживает Web Audio API, молча игнорируем
    }
}
